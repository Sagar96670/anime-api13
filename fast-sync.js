...
function extractCatalog(
  ...
  $("a[href]").each((i, el) => {
    ...
    if (seriesSlug) {
      ...
      if (ongoingSeriesMap) {
        ongoingSeriesMap.set(seriesSlug, href);
      }
    }
...
async function discoverCatalog() {
  ...
  const queue = CATEGORY_SEEDS.map(seed => ({
    url: new URL(seed, BASE).href,
    ongoing:
      seed === "/category/status/ongoing/"
  }));

  const series = new Map();
  const movies = new Map();
  const ongoingSeries = new Map();

  while (queue.length) {
    const current = queue.shift();

    const url = current.url;
    const isOngoing = current.ongoing;
    ...
    try {
      ...
      console.log(
        `   Series=${series.size} Movies=${movies.size} Ongoing=${ongoingSeries.size} Queue=${queue.length}`
      );
...
async function incrementalSync() {
  console.log("\n==========================================");
  console.log("INCREMENTAL ANIME SALT SYNC");
  console.log("==========================================");
  ...
  const indexes = {
    anime: new Map(
      db.anime.map((item, index) => [item.id, index])
    ),
    ...
    movies: new Map(
      db.movies.map((item, index) => [item.id, index])
    )
  };

  console.log(
    `Existing DB: anime=${db.anime.length}, episodes=${db.episodes.length}, movies=${db.movies.length}`
  );
  ...
  const seriesTargets = new Map();

  // New series: sync regardless of category.
  for (const [slug, url] of catalog.series.entries()) {
    if (!indexes.anime.has(slug)) {
      seriesTargets.set(slug, {
        url,
        isNew: true
      });
    }
  ...
  // Existing series: refresh only if found in ongoing category.
  for (const [slug, url] of catalog.ongoingSeries.entries()) {
    if (indexes.anime.has(slug)) {
      seriesTargets.set(slug, {
        url,
        isNew: false
  ...
  console.log(
    `Series selected: ${seriesTargets.size}`
  );

  for (const [slug, target] of seriesTargets.entries()) {
    if (target.isNew) {
      console.log(`[NEW SERIES] ${slug}`);
    } else {
      console.log(`[ONGOING UPDATE] ${slug}`);
    }

    const ok = await syncSeries(
      db,
      slug,
      target.url,
      indexes
  ...
  for (const [slug, url] of catalog.movies.entries()) {
    if (indexes.movies.has(slug)) {
      skippedMovies++;
      continue;
    }

    console.log(`[NEW MOVIE] ${slug}`);

    const ok = await syncMovie(
      db,
      slug,
      url,
      indexes
  ...
  console.log("Updated ongoing series:", updatedSeries);
  console.log("New movies:", newMovies);
  console.log("Skipped existing movies:", skippedMovies);
  console.log("Failed:", failed);

  console.log("Database anime:", db.anime.length);
  console.log("Database episodes:", db.episodes.length);
  console.log("Database movies:", db.movies.length);
